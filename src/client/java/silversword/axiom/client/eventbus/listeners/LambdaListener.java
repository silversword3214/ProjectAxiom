...
public class LambdaListener implements IListener {
    public interface Factory {
        MethodHandles.Lookup create(Method lookupInMethod, Class<?> axiomClass) throws InvocationTargetException, IllegalAccessException;
    }
    ...
    public LambdaListener(Factory factory, Class<?> axiomClass, Object object, Method method) {
        this.target = method.getParameters()[0].getType();
        this.isStatic = Modifier.isStatic(method.getModifiers());
        this.priority = method.getAnnotation(AxiomEvent.class).priority();
        ...
        try {
            ...
            if (isJava1dot8) {
                ...
                lookup = lookupConstructor.newInstance(axiomClass);
                lookupConstructor.setAccessible(a);
            }
            else {
                lookup = factory.create(privateLookupInMethod, axiomClass);
            }
            ...
            if (isStatic) {
                methodHandle = lookup.findStatic(axiomClass, name, methodType);
                invokedType = MethodType.methodType(Consumer.class);
            }
            else {
                methodHandle = lookup.findVirtual(axiomClass, name, methodType);
                invokedType = MethodType.methodType(Consumer.class, axiomClass);
            }
